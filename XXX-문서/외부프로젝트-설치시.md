...
2. **파일 이동 및 설치 (최초 1회)**
   ...
   # 테스트 용으로 잠시 사용 ㅇㅇㅇ
   cp ../so-grid-core/so-grid-core-0.1.0.tgz /Users/danyoh/XXX-AI/sr-mobile/nextjs-client/packages/
   cp ../so-grid-react/so-grid-react-0.1.0.tgz /Users/danyoh/XXX-AI/sr-mobile/nextjs-client/packages/
   
   # 1. 기존 패키지 제거 (필수: 캐시 꼬임 방지)
   pnpm remove so-grid-core so-grid-react
   
   # (선택) 완전히 깨끗하게 다시 설치하려면:
   # rm -rf node_modules pnpm-lock.yaml

   # 2. 두 패키지를 함께 설치 (의존성 연결)
   pnpm install ./libs/so-grid-core-0.1.0.tgz ./libs/so-grid-react-0.1.0.tgz
